...
times = 50
K = rnorm(mean = 250, sd = 50, n = times) # Set a variance of SD 1
r = rnorm(mean = 0.01, sd = 1, n = times)
g <- 2  # Linear growth rate after canopy closure (kg/year)

#K2 <- runif(n = times, min = 200, max = 400) Maybe we can Run this 

possibility1_df = cbind.data.frame(r=r, K=K)
possibility2_df = cbind.data.frame(r=r, K=K)
...
sensitivity_C = sobolSalt(model = NULL, X1, X2, nboot = 300)
...
# lets add names 
colnames(sensitivity_C$X) = c("r","K")
```

Now lets apply the ODE with the created values

```{r}
source("/Users/javipatron/Documents/MEDS/Courses/eds230/assignments/assignment6/eds230-carbon-forest-model/functions/equation1.R")
